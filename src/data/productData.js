...
function getImgUrl(fileName, type, isMain, isCompressed = false) {
  ...
  const imgURL = `/assets/img/${subfolder}/${fileName}-${number}.${ext}`;
...
export const eyeglassesData = framesData.map((item) => Object.freeze(mapImage(item)));

export const sunglassesData = sunniesData.map((item) => Object.freeze(mapImage(item)));
